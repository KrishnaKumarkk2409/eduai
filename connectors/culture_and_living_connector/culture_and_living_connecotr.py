...
# Configure logging
logging.basicConfig(filename='culture_and_living_vogue_data.log', level=logging.INFO,
...
def create_table(connection):
    try:
        ...
        connection.commit()
        logging.info("Table 'culture_and_living_vogue' created or already exists.")
    except Error as e:
...
def insert_article(connection, title, date_of_published, summary, link, image, image_link, content):
    try:
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO culture_and_living_vogue (title, date_of_published, summary, link, image, image_link, content)
...
current_page = f"{base_url}/culture-and-living"
...
print("Table 'culture_and_living_vogue' ensured.")
