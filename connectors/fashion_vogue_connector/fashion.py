...
def create_table(connection):
    try:
        cursor = connection.cursor()
        create_table_query = """
        CREATE TABLE IF NOT EXISTS fashion_vogue (
        ...
        connection.commit()
        logging.info("Table 'fashion_vogue' created or already exists.")
    except Error as e:
...
def insert_article(connection, title, date_of_published, summary, link, image, image_link, content):
    try:
        cursor = connection.cursor()
        insert_query = """
        INSERT INTO fashion_vogue (title, date_of_published, summary, link, image, image_link, content)
...
current_page = f"{base_url}/fashion/fashion-trends"
...
print("Table 'fashion_vogue' ensured.")
...
while current_page:
    ...
    next_page_button = soup.find('a', class_='BaseButton-bLlsy ButtonWrapper-xCepQ cRxydS giwzDJ button button--primary')
